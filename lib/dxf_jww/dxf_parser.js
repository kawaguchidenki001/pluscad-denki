 ...
 *      TEXT/MTEXT/POINT/LWPOLYLINE/POLYLINE/INSERT/HATCH/SOLID/ELLIPSE/SPLINE/DIMENSION）, BLOCKS定義
 *
...
function readEntityBody(parser, type) {
    ...
    let polyCur = null;
    // SPLINE: 制御点(10/20)・フィット点(11/21)を複数蓄積
    let splCur = null, fitCur = null;
    if (type === 'SPLINE') { ent.controlPoints = []; ent.fitPoints = []; }
    while (!parser.eof()) {
        const pair = parser.nextPair();
        if (!pair) break;
        const [code, val] = pair;
        ...
        if (code === 8) ent.layer = val;
            ...
            if (type === 'LWPOLYLINE' || type === 'HATCH') {
                ...
                if (splCur !== null) ent.controlPoints.push(splCur);
                splCur = { x: parseFloat(val), y: 0 };
            } else ent.x = parseFloat(val);
        }
        else if (code === 20) {
            if (type === 'LWPOLYLINE' || type === 'HATCH') { if (polyCur) polyCur.y = parseFloat(val); }
            else if (type === 'SPLINE') { if (splCur) splCur.y = parseFloat(val); }
            else ent.y = parseFloat(val);
        }
        else if (code === 11) {
            if (type === 'SPLINE') {
                if (fitCur !== null) ent.fitPoints.push(fitCur);
                fitCur = { x: parseFloat(val), y: 0 };
            } else ent.x2 = parseFloat(val);
        }
        else if (code === 21) {
            if (type === 'SPLINE') { if (fitCur) fitCur.y = parseFloat(val); }
            else ent.y2 = parseFloat(val);
        }
        else if (code === 40) {
            if (type === 'ELLIPSE') ent.ratio = parseFloat(val);   // 短軸/長軸比
            else ent.r = parseFloat(val);                          // 半径 / 文字高さ
        }
        else if (code === 41) {
            if (type === 'INSERT') ent.xScale = parseFloat(val);
            else if (type === 'ELLIPSE') ent.startParam = parseFloat(val);  // 楕円弧の開始媒介変数
            else ent.widthFactor = parseFloat(val);    // TEXT/MTEXTの横倍率
        }
        else if (code === 42) {
            if (type === 'INSERT') ent.yScale = parseFloat(val);
            else if (type === 'ELLIPSE') ent.endParam = parseFloat(val);    // 楕円弧の終了媒介変数
        }
        else if (code === 50) {
    ...
    if (type === 'SPLINE') { if (splCur) ent.controlPoints.push(splCur); if (fitCur) ent.fitPoints.push(fitCur); }
    return ent;
}
