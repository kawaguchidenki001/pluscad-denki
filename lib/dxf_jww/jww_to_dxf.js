...
class DxfWriter {
    ...
    writeTables() {
        ...
        // LTYPE 表（実線＋破線/点線/一点鎖線。パターン長は実寸mmのモデル空間値）
        const LT = [
            { n:'CONTINUOUS', d:'Solid line',      pat:[] },
            { n:'DASHED',     d:'Dashed _ _ _',    pat:[120,-80] },
            { n:'DOT',        d:'Dot . . .',       pat:[0,-80] },
            { n:'DASHDOT',    d:'Dash dot _._._',  pat:[200,-80,0,-80] },
        ];
        this.pair(0, 'TABLE'); this.pair(2, 'LTYPE'); this.pair(70, LT.length);
        for (const t of LT) {
            this.pair(0, 'LTYPE'); this.pair(2, t.n); this.pair(70, 0);
            this.pair(3, t.d); this.pair(72, 65); this.pair(73, t.pat.length);
            this.pair(40, t.pat.reduce((s,v)=>s+Math.abs(v),0));
            for (const v of t.pat) this.pair(49, v);
        }
        this.pair(0, 'ENDTAB');
    ...
    line(x1,y1,x2,y2,layer,color,ltype) {
        this.pair(0,'LINE');this.pair(8,layer);
        if(ltype && ltype!=='CONTINUOUS') this.pair(6,ltype);   // 線種（LTYPE表に定義済みのもの）
        this.pair(62,color);
